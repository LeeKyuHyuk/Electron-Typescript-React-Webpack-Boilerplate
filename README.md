...
아래 명령어로 개발 모드로 Webpack을 시작합니다. 코드가 변경되면, 실시간으로 다시 빌드됩니다.

```sh
npm run dev
```

`npm run dev`를 사용하면 빌드만 하고, 애플리케이션은 실행하지 않습니다. 아래 명령어로 애플리케이션을 실행합니다.
...
npm install --save-dev copy-webpack-plugin cross-env css-loader electron electron-builder file-loader html-webpack-plugin lodash rimraf source-map-loader style-loader ts-loader ts-node typescript webpack webpack-cli
```
